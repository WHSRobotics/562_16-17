...
public class IMUBNO055
{
     ...
     */
    public static IMUBNO055 getInstance(opmode_t mode, vector_type_t vectorType) {
        return getInstance(mode, vectorType); // Removed , I2C.Port.kOnboard, BNO055_ADDRESS_A
    }
     ...
     */
    private void update() {
        currentTime = System.nanoTime(); //seconds
        if(!initialized) {
...
//					+ ", next: " + nextTime);
            ...
            switch(state) {
                case 0:
                    ...
                    if((0xFF & read8(reg_t.BNO055_CHIP_ID_ADDR)) != BNO055_ID) {
                        ...
                        sensorPresent = true;
                        state++;
                        nextTime = System.nanoTime() + 0.050;
                    }
                    break;
                case 1:
                ...
                case 6:
                    if(currentTime >= nextTime) {
                        ...
                        nextTime = System.nanoTime() + 0.500;
                        state++;
                    }
                    break;
                case 7:
    ...
    private class IMUBNO055UpdateTask extends TimerTask {
        private IMUBNO055 imu;

        private IMUBNO055UpdateTask(IMUBNO055 imu) {
            if (imu == null) {
